...
from .models import Pelanggan, Slide, Perbaikan, DetailPasang, SettingPembayaran, Produk,Tahun, Teknisi
from django.contrib.auth.models import User
from django.contrib.auth.models import Group
from datetime import datetime, timedelta, time
today = datetime.now().date()
from .forms import TeknisiForm, DetailPasangForm, TahunForm, ProdukForm, SlideForm, PelangganForm,UserForm,PerbaikanForm
...
@login_required(login_url='login_page')
@ijinkan_pengguna(yang_diizinkan=['administrator']) 
def teknisi_admin(request):
    teknisi = Teknisi.objects.all()
    context = {
        'data': teknisi,
        'judul': 'Halaman Teknisi Wifi',
        'menu': 'teknisi',
    }
    return render(request, 'admin_teknisi.html', context)

@login_required(login_url='login_page')
@ijinkan_pengguna(yang_diizinkan=['administrator']) 
def formteknisi_admin(request):
    form = TeknisiForm()
    formuser = UserForm()
    if request.method == 'POST':
        username = request.POST['username']
        ...
        akses = Group.objects.get(name='teknisi')
        user.groups.add(akses)

        formsimpan = TeknisiForm(request.POST)
        if formsimpan.is_valid():
            data = formsimpan.save()
            data.user = user
            data.save()
            return redirect('teknisi_admin')
    context = {
        'judul': 'Halaman Form Teknisi',
        'menu': 'teknisi',
        'form':form,
        'formuser':formuser
    }
    return render(request, 'admin_formteknisi.html', context)

@login_required(login_url='login_page')
@ijinkan_pengguna(yang_diizinkan=['administrator']) 
def editteknisi_admin(request, pk):
    teknisi = Teknisi.objects.get(id=pk)
    user = User.objects.get(id=teknisi.user.id)
    form = TeknisiForm(instance=teknisi)
    formuser = UserForm(instance=user)
    if request.method == 'POST':
        formsimpan = TeknisiForm(request.POST,request.FILES, instance=teknisi)
        if formsimpan.is_valid():
            formsimpan.save()
            return redirect('teknisi_admin')
    context = {
       'judul': 'Halaman Form Edit Teknisi',
        'menu': 'teknisi',
        'form':form,
        'formuser':formuser
    }
...
@login_required(login_url='login_page')
@ijinkan_pengguna(yang_diizinkan=['administrator']) 
def deleteteknisi_admin(request, pk):
    hapus = Teknisi.objects.get(id=pk)
    if request.method == 'POST':
        hapus.delete()
        return redirect('teknisi_admin')
    context = {
        'judul': 'Halaman Hapus Teknisi',
        'menu': 'teknisi',
        'hapus':hapus  
    }
    return render(request, 'admin_hapusteknisi.html', context)
