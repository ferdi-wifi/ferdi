...
def pasangproduk (request, slug):
    produk = get_object_or_404(Produk, slug=slug)
    context = {
        'judul': 'Halaman  Produk Wifi Qiana',
        'menu': 'beranda',
        'produkdetail':produk,
    }
...
@login_required(login_url='login_page')
@ijinkan_pengguna(yang_diizinkan=['pelanggan'])     
def datapasangva (request, id):
    idpelanggan = request.user.pelanggan.id
    pelanggan = Pelanggan.objects.get(id=idpelanggan)
    pasang = DetailPasang.objects.get(pelanggan__id=pelanggan.id, id=id)
    ...
    context = {
         'judul': 'Halaman Pemasangan Produk Wifi Qiana',
        'pasang':pasang,
...
def pembayaranbulanan (request):
    idpelanggan = request.user.pelanggan.id
    pelanggan = Pelanggan.objects.get(id=idpelanggan)
    data = SettingPembayaran.objects.filter(detailpasang__pelanggan__id=pelanggan.id).order_by('-id')
    context = {
...
@login_required(login_url='login_page')
@ijinkan_pengguna(yang_diizinkan=['pelanggan'])     
def databayarva (request, id):
    iddetail = get_object_or_404(SettingPembayaran, id=id)
    idpelanggan = request.user.pelanggan.id
    pelanggan = Pelanggan.objects.get(id=idpelanggan)
    bayar = SettingPembayaran.objects.get(detailpasang__pelanggan__id=pelanggan.id, id=id)
    ...
    context = {
        'judul': 'Halaman Pembayaran bulanan Produk Wifi Qiana',
